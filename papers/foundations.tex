...
\emph{Status of this Part: foundational commitments---the postulates and definitions on which everything else rests, together with the closure geometry they induce. Nothing here is derived; everything here is priced in the ledger of Section~\ref{sec:ledger}.}
...
\begin{postulate}[Closure laws and closure distance]\label{post:closure}
Correlations among degrees of freedom may imply additional correlations. Closure laws express this implication structure and define refinement relations among records, $r_1 \preceq r_2$, forming a refinement partial order whose chains are admissible refinement sequences. This structure carries a directed distinguishability measure, closure distance $d_c(r \to r')$, quantifying the minimal physical restructuring required to refine $r$ into $r'$. Closure distance is postulated to satisfy $d_c(r \to r) = 0$ and the directed triangle inequality $d_c(r \to r'') \le d_c(r \to r') + d_c(r' \to r'')$; it is a quasi-metric---refinement is directional, and no symmetry $d_c(r \to r') = d_c(r' \to r)$ is assumed.
\end{postulate}

\begin{postulate}[Closure quantum]\label{post:quantum}
Closure distance accumulates discretely: along any refinement chain, $d_c = N_{\mathrm{steps}}\,\delta_c$, where $\delta_c$ is the closure quantum, the minimal distinguishable increment of correlation-encoding capacity. Nothing fundamental is infinitesimal; all continuum descriptions of closure geometry are coarse-grained.
\end{postulate}

\begin{remark}
Discreteness is a major physical commitment and is stated as its own postulate rather than as a rider. The conjecture that $\delta_c$ is Planck-scale---suggested by the horizon-entropy material of Section~\ref{sec:dynamics}---is exactly that, a conjecture, and is priced as such in the ledger.
\end{remark}
...
\begin{remark}[What this postulate actually costs]\label{rem:obscost}
The postulate's definitional clauses---what ``stage,'' ``chain,'' and ``measurement'' mean---are vocabulary, and vocabulary cannot be false. Its substantive commitment is the italicized existence clause, matching the companion paper's Postulate of the same name \cite{TillittBorn}: that configurations containing refinement-ordered observer stages exist at all. A model of Postulates~\ref{post:omega}--\ref{post:quantum} could lack them, and then nothing downstream that is observer-relative---emergent time, probability, the whole of Parts II--III as read by anyone---would have a subject. The ledger prices the existence clause separately for this reason; the explicit model of Part II realizes chains by construction.
\end{remark}
...
\emph{Status of this Part: direct consequences of the ontology and the Part II results. Section~\ref{sec:entropy} is close to theorem-grade; Sections~\ref{sec:complexity}--\ref{sec:self} are structural arguments, priced as such in the ledger.}
...
\emph{Status of this Part: framework-supported conjectures and scheduled derivations---claims with structural plausibility and, where noted, a derivation path, but without the theorem status of Part II. Stating the difference is deliberate. Each conjecture below is a candidate for the Part 0 method: a structure which, taken seriously, would dictate further features of the ontology.}
...
\emph{The classical limit and cosmology.} Effectively classical macroscopic behavior follows from the stability results at the argument level (Section~\ref{sec:classical}); cosmological consistency---expansion, background structure---is at present an unexamined target, listed for completeness and honesty.

\emph{Distinctive predictions.} Three commitments distinguish the framework from continuum quantum mechanics, all consequences of Part II and stated in indicative mood. (1) \emph{Rational probabilities}: every probability is $m/N$, so Born statistics hold to accuracy $O(1/N)$---empirically invisible for astronomical $N$, falsifiable in principle. (2) \emph{Maximum entropy}: no record's entropy exceeds $\log|\Omega|$, aligning natively with the Bekenstein and de Sitter bounds that motivate finiteness. (3) \emph{Tsirelson deviations of order $1/N$}: the counted CHSH quantity differs from $2\sqrt{2}$ by $O(1/N)$---the scale is robust, following from rationality of counted correlators alone, while the sign (the counted value can land \emph{above} the bound, as it does in the model) is conditional on the integer-apportionment rule, a model postulate whose selection by the ontology is open. Even so, a possible excess over Tsirelson by an amount set by the world's denominator is a statement no continuum formulation makes, and in principle an experimental lower bound on $N$ \cite{TillittCHSH}.

\section{The Frontier}\label{sec:frontier}
The open problems, in descending order of centrality. \emph{First, the arrow problem}: deriving [LC]---the existence of arrows and linear composition---from deeper CR primitives; the relational phase postulate supplies [LC]'s meaning (record-level uniformity of phase increments) but not its derivation, and the stability results show what selects among arrow structures, not why arrows exist. \emph{Second, the continuum limit}: the formal construction of $g_{ab}$ (and the Schr\"odinger kernel, and the Lorentzian signature) from the discrete closure lattice, on which all of Part IV depends---including the causal-set bridge question of Section~\ref{sec:causal}. \emph{Third}, the base-level phase texture: what determines the stage-relative increments $\Delta S$ that [LC] organizes, and that generic admissible textures exist; with this belongs the origin of [AS] and of the integer-apportionment rule the model postulates. \emph{Fourth}, the $\delta_c$--entropy statement gating the countable form of the arrow of time (Section~\ref{sec:entropy}). Phase additivity requires no entry here, being a lemma of Postulate~\ref{post:omega}; the Bell question is answered by the second companion \cite{TillittCHSH}. Each remaining item is research, not patchwork; none undermines the Part II results, which are conditional on assumptions stated where they are used.
...
Closure distance, quasi-metric properties & P & Post.~\ref{post:closure}\\
Closure quantum $\delta_c$ (discreteness) & P & Post.~\ref{post:quantum}\\
...
$O(1/N)$ Born deviation; $\log|\Omega|$ cap & Prediction & \S\ref{sec:targets}\\
Tsirelson deviation: $O(1/N)$ scale & Prediction (robust) & \S\ref{sec:targets}\\
Tsirelson deviation: sign (two-sidedness) & Conditional on apportionment rule & \S\ref{sec:targets}\\
